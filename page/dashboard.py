...
# Stripe API keys
stripe.api_key = "sk_test_51OgR25SAPXXhEXnhLliZnQoHRi56F7hXMVuqIZMPYLxukNsRsSbNh1gwkAXpv8ghurxtvr08X1QvhBzslkE8N3M500AOSycF1l"
STRIPE_PUBLIC_KEY = "pk_test_51OgR25SAPXXhEXnh24uDoaBYr7jkPPGX9R9JJqKSy5BCg2MTGImvNmrKtXKHimzKCyzQSztLkbO3tLnwaWYrCFqs00nl4WaqaK"
...
option = st.sidebar.radio("", ("Dashboard", "Single Call", "Bulk Call", "Call Logs", "Transcripts", "Configuration"))
    ...
    if missing_keys:
        ...
        if st.button("Make Call"):
            if phone_number and user_prompt:
                response = requests.get(f"http://localhost:5000/make_call?phone_number={phone_number}&prompt={user_prompt}")
...
# Credits content
    ...
    with st.form(key='credits_form'):
        ...
        if submit_button:
            # Create a Stripe Checkout session dynamically based on user input
            try:
                session = stripe.checkout.Session.create(
                    ...
                    line_items=[{
                        'price_data': {
                            ...
                            'unit_amount': int(credits * 100),  # Convert credits to cents
                        },
                        'quantity': 1,
                    }],
                    mode='payment',
                    success_url='https://your-success-url',  # Replace with your success URL
                    cancel_url='https://your-cancel-url',    # Replace with your cancel URL
                )
                st.write(f"You are about to add {credits} credits.")
                sleep(2)  # Simulate delay for user to see the message

                # Redirect to the newly created Stripe Checkout session URL
                st.markdown(f"[Proceed to Payment]({session.url})", unsafe_allow_html=True)
